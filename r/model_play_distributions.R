...
xgb.save(to_model, "to_model.model")
...
# Saving the model (optional)
xgb.save(to_yrdline_model, "to_yrdline_model.model")
...
# Saving the model (optional)
xgb.save(no_to_yrdline_model, "no_to_yrdline_model.model")
...
#all data 
predict_play_dist <- test_data %>% slice(1)
#predicted data row
...
# Feed Yardline Predictions to EP Model -----------------------------------
...
pred_next_play_tibble <- pred_tibble %>% 
  ...
  mutate(posteam2 = ifelse(posteam_label == 'team', posteam, defteam),
